 ...
 * @returns 
 */
export const userService = async (username, fullName, email, password, avatarLocalPath) => {
 //check if user already exists: username, email 
   const existingUser = await checkExistingUser(email, username);

   if(existingUser){
    throw new ApiError(409, "User Already registered. Please log in or use a different email/username")
   }
   ...
   let userCreated = await createdUser({username, fullName, email, password, avatar: avatarRes.secure_url});
